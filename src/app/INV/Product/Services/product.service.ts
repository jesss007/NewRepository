...
export class ProductService {
  ...
  private getDropdownUrl = 'https://localhost:7065/api/Product/ProductDropdown'
  ...
  deleteProduct(data: ProductDelete): Observable<ApiResponse<Product>> {
    return this.http.delete<ApiResponse<Product>>(
      `${this.deleteUrl}?Id=${data.id}&DeletedBy=${data.deletedBy}`,
    );
  }
  ...
  ): Observable<ApiResponse<MvGridConfig<Product>>> {
    return this.http.get<ApiResponse<MvGridConfig<Product>>>(
  ...
  getProductsDropdown():Observable<ApiResponse<ProductDropdown[]>>{
    return this.http.get<ApiResponse<ProductDropdown[]>>(this.getDropdownUrl)
  }
}
